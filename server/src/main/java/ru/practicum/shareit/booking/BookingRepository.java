...
public interface BookingRepository extends JpaRepository<Booking, Long> {
    ...
    @Query("SELECT b FROM Booking b WHERE b.booker.userId = :bookerId AND b.startTime <= :now AND b.endTime >= :now ORDER BY b.startTime " +
            "DESC")
    ...
    @Query(value = "SELECT * FROM bookings b WHERE b.item_id = :itemId AND b.start_time > CURRENT_TIMESTAMP ORDER BY b.start_time ASC " +
            "LIMIT 1",
            nativeQuery = true)
